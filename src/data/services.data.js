import ServiceImg1 from 'assets/images/services/service1.webp';
import ServiceImg2 from 'assets/images/services/service2.webp';
import ServiceImg3 from 'assets/images/services/service3.webp';
import ServiceImg4 from 'assets/images/services/service4.webp';
import ServiceImg6 from 'assets/images/services/service6.webp';
import ServiceImg7 from 'assets/images/services/service7.webp';
import ServiceImg8 from 'assets/images/services/service8.webp';

export const servicesData = [
  {
    id: '1',
    language: 'ru',
    created: '11/25/2024',
    title: 'Международные ж/д грузоперевозки',
    subtitle: 'Международные железнодорожные грузоперевозки с Орион-Транс',
    images: { file: ServiceImg1, name: 'wagons' },
    content: [
      {
        text: 'Железнодорожные грузоперевозки — это перевозка грузов на большие расстояния оперативно и хорошим соотношением цена-качество-сроки. Компания ОсОО “Орион-Транс” с более чем 10-летним опытом работы в сфере логистики и экспедирования обеспечивает полный цикл перевозки: от расчета и оплаты железнодорожных тарифов до оформления всех необходимых документов. Наши специалисты подберут оптимальное решение для вашего груза, учитывая все особенности транспортировки. Орион-Транс сотрудничает с надежными представителями железных дорог стран СНГ и других регионов, что гарантирует безопасность и надежное сотрудничество.',
        title:
          'Что Вам необходимо знать об услугах международных железнодорожных грузоперевозках Орион-Транс?',
      },
      {
        title: 'Почему именно перевозка железнодорожным транспортом?',
        text: 'Железнодорожный транспорт — это идеальный выбор для перевозки тяжелых и габаритных грузов, таких как сырья, оборудование и продукция, он обеспечивает большой объем перевозки и высокую степень безопасности. Выбирая Орион-Транс, вы получаете профессиональную поддержку на каждом этапе, конкурентоспособные тарифы и качественное обслуживание. Мы работаем для того, чтобы ваша логистика была простой, надежной и удобной.',
      },
      {
        title: 'Почему именно наши услуги по ж/д перевозкам?',
        text:
          '1. Мы напрямую имеем договора со всеми администрациями железных дорог региональных стран и имеем там своих представителей. Наш 15-летний опыт подтверждает наш высокий профессионализм и ответственность.\n' +
          '2. Мы организуем перевозку любых грузов в любом объеме и проконтролируем весь процесс транспортировки, предоставляя актуальную информацию о местонахождении груза.\n' +
          '3. Вы получите подробную консультацию по каждому этапу, где предварительно рассчитаем стоимость железнодорожного тарифа для любых направлений бесплатно\n' +
          '4. Качество наших услуг неизменно высокое благодаря опытным специалистам, готовым помочь вам в любое время.\n' +
          '5.Мы предварительно можем брать на себя оплату всех расходов: аренду вагонов, оформление документов, охрану и дополнительные сборы, избавляя вас от лишних затрат и сложностей, которые могли бы возникнуть, если бы вы это делали самостоятельно.\n' +
          '6. Предоставим подходящий тип вагона по вашему запросу: крытые вагоны, полувагоны, цистерны, платформы и контейнеры, а также предоставим оптимальные маршруты для доставки вашего груза.\n',
      },
    ],
  },
  {
    id: '1',
    language: 'en',
    created: '11/25/2024',
    title: 'International Rail Freight Transport',
    subtitle: 'International Rail Freight Transport',
    images: { file: ServiceImg1, name: 'wagons' },
    content: [
      {
        text: "What you need to know about Orion-Trans' international railway freight services",
        title:
          'Rail freight transportation is an efficient way to move goods over long distances with a good balance of price, quality, and delivery time. Orion-Trans, with over 10 years of experience in logistics and forwarding, ensures the complete transportation cycle—from tariff calculations and payments to the necessary documentation. Our specialists will offer the best solution for your cargo, considering all aspects of its transportation. Orion-Trans partners with reliable representatives of the railway networks in the CIS countries and other regions, guaranteeing safety and reliability.',
      },
      {
        title: 'Why Choose Rail Freight Transportation?',
        text: 'Rail freight is an ideal choice for transporting heavy and oversized goods such as raw materials, equipment, and products. It offers high shipping volumes and a high level of safety. By choosing Orion-Trans, you benefit from professional support at every stage, competitive tariffs, and quality service. We work to make your logistics simple, reliable, and convenient.',
      },
      {
        title: 'Why Choose Our Rail Freight Services?',
        text: '',
      },
      {
        title: 'Reliable Partnerships:',
        text: 'We have direct agreements with reliable railway service providers in multiple countries, ensuring secure and efficient transportation. Our 15 years of experience confirm our high professionalism and responsibility.',
      },
      {
        title: 'Comprehensive Cargo Transport Solutions',
        text: 'We organize the transportation of any type and volume of cargo, managing the entire process and providing up-to-date information about your cargo’s location.',
      },
      {
        title: 'Free Consultation & Tariff Calculations',
        text: 'You will receive detailed consultation on every stage of transportation. We offer free preliminary rail tariff calculations for any route.',
      },
      {
        title: 'High Quality of Service',
        text: 'Our services are consistently of high quality, thanks to our experienced specialists who are ready to assist you at any time.',
      },
      {
        title: 'Tailored Wagon Selection',
        text: 'We will provide the right type of wagon for your needs—covered wagons, gondola cars, tank wagons, platforms, or containers—and offer optimal routes for delivering your cargo.',
      },
    ],
  },
  {
    id: '1',
    language: 'zh',
    created: '11/25/2024',
    title: 'Orion-Trans 国际铁路货物运输',
    subtitle: '',
    images: { file: ServiceImg1, name: 'wagons' },
    content: [
      {
        text: '铁路货物运输是通过铁路进行长途货物运输，具有高效、良好的性价比和时间要求。Orion-Trans公司在物流和货运代理领域拥有超过10年的经验，可提供一系列的运输服务：从铁路运费的计算和支付到所有必要文件的办理。我们的专业人员会根据您的货物特点，选择最合适的运输方案。Orion-Trans与独联体国家及其他地区的可靠铁路公司合作，确保合作安全可靠。',
        title: '关于 Orion-Trans 国际铁路货运服务，您需要了解哪些信息？',
      },
      {
        title: '为什么选择铁路运输？',
        text: '铁路运输是运输原材料、设备和产品等大宗重型货物的理想选择，运输量大，安全性高。选择Orion-Trans，您将获得在每个环节的专业支持、具有竞争力的价格和优质的服务。我们致力于让您的物流变得简单、可靠和便捷。',
      },
      {
        title: '为什么选择我们的铁路运输服务？',
        text:
          '1. 我们与所有区域国家的铁路管理部门直接签订合同，并在当地设有代表处。我们15年的经验证明了我们的高专业水平和责任感。' +
          '2. 我们可以运输任何货物，无论数量多少，并且全程控制运输过程，提供货物当前位置的实时信息。' +
          '3. 我们将为您提供每个阶段的详细咨询，并免费预先计算任何路线的铁路运费。' +
          '4. 我们的服务质量始终如一，得益于经验丰富的专业人员，我们随时准备为您提供帮助。' +
          '5. 我们可以预先承担所有费用：车厢租赁、文件办理、安全保障和额外费用，为您免除自行处理可能产生的额外支出和复杂问题。' +
          '6. 我们将根据您的需求提供合适的车厢类型：封闭车厢、敞车、油罐车、平板车和集装箱运输车，并为您的货物提供最佳运输路线。',
      },
    ],
  },

  {
    id: '2',
    language: 'ru',
    created: '11/25/2024',
    title: 'Экспедирование грузов',
    subtitle:
      'Расчет и оплата ж/д тарифов на экспортно-импортные и транзитные перевозки',
    images: { name: '', file: ServiceImg2 },
    content: [
      {
        title: 'Что Вам нужно знать об экспедировании грузов?',
        text: 'Железнодорожное экспедирование грузов является достаточно трудоемкой процедурой, которая координирует и контролирует движение грузов по железной дороге. Этот процесс требует профессионального подхода, чтобы минимизировать простои, задержки и дополнительные расходы. Экспедирование становится особенно актуальным для международных и транзитных перевозок, где важны точность и координация. На сегодняшний день железнодорожное экспедирование грузов остается одним из наиболее экономически выгодных способов транспортировки, подходящим для различных типов грузов, включая порожние и груженые вагоны, а также контейнеры.',
      },
      {
        title: 'Что включает в себя услуга ж/д экспедирования грузов?',
        text: 'Наши транспортно-экспедиторские услуги охватывают экспортно-импортные и транзитные перевозки. Мы экспедируем различные виды грузов, порожних и груженых вагонов/контейнеров, включая насыпные, наливные, контейнерные, опасные и другие - по территориям Узбекистана (УТИ), Кыргызстана (КРГ), Туркменистана (ТРК), Таджикистана (ТДЖ), Казахстана (КЗХ), Китая (КНР), Афганистана (ИГА), а также Ирана (ИРА) и России (РЖД). Железнодорожное экспедирование груза включает в себя расчет ж/д перевозки, где уже учитываются расходы по дополнительным сборам и стоимости охраны. Для этого мы используем специализированную программы для соответствующего расчета тарифа на перевозку грузов. Так как каждая перевозка индивидуальна и требует определенного подхода, мы принимаем во внимание маршрут, вид груза, подвижной состав и подготавливаем все необходимые документы для сопровождения груза, а также согласовываем перемещение товара на всех этапах маршрута. По запросу клиента мы можем организовать охрану груза – это дополнительная услуга, которую мы оказываем через наших проверенных партнеров.',
      },
      {
        title:
          'Благодаря нашему опыту и высокому профессионализму мы делаем все возможное, чтобы ваша транспортировка прошла качественно.',
        text: '',
      },
      {
        title:
          'Обращаясь к нам, вы получаете надежного партнера для экспедирования грузов по выгодным условиям!',
        text: '',
      },
    ],
  },
  {
    id: '2',
    language: 'en',
    created: '11/25/2024',
    title: 'Railway Freight-Forwarding',
    subtitle:
      'Calculation and Payment of Rail Tariffs for Export, Import, and Transit Tariffs',
    images: { name: '', file: ServiceImg2 },
    content: [
      {
        title: 'What you need to know about freight forwarding',
        text: 'Rail freight forwarding is a complex procedure that coordinates and monitors the movement of goods by rail. This process requires a professional approach to minimize delays, downtime, and additional costs. Freight forwarding is especially crucial for international and transit shipments, where accuracy and coordination are key. Today, rail freight forwarding remains one of the most cost-effective methods of transportation, suitable for a wide variety of cargo types, including empty and loaded wagons, as well as containers.',
      },
      {
        title: 'What does rail freight forwarding include?',
        text: 'Our freight forwarding services cover export, import, and transit shipments. We handle various types of cargo, including bulk, liquid, containerized, hazardous, and other goods, as well as empty and loaded wagons/containers, across territories including Uzbekistan (UTI), Kyrgyzstan (KRG), Turkmenistan (TKM), Tajikistan (TJK), Kazakhstan (KAZ), China (PRC), Afghanistan (AFG), Iran (IRN), and Russia (RZD).',
      },
      {
        title: '',
        text: 'Rail freight forwarding includes the calculation of rail transport costs, which takes into account additional fees and security expenses. For this, we use specialized software to calculate the most accurate tariff for your shipment. Since every transportation job is unique and requires a tailored approach, we consider factors such as the route, type of cargo, rolling stock, and prepare all the necessary documents for cargo accompaniment. We also coordinate the movement of goods at every stage of the journey.',
      },
      {
        title: '',
        text: 'Upon request, we can arrange cargo security through our trusted partners, which is an additional service we offer to ensure the safety of your shipment.',
      },
      {
        title: '',
        text: 'Thanks to our experience and high professionalism, we do everything possible to ensure your transport is carried out smoothly and efficiently.',
      },
      {
        title: '',
        text: 'By choosing us, you gain a reliable partner for freight forwarding at favorable terms!',
      },
    ],
  },
  {
    id: '2',
    language: 'zh',
    created: '11/25/2024',
    title: '货运代理',
    subtitle: '出口、进口及过境运输的铁路运费计算与支付',
    images: { name: '', file: ServiceImg2 },
    content: [
      {
        title: '关于货运代理，您需要了解什么？',
        text: '铁路货运代理是一项相对繁琐的程序，负责协调和控制货物在铁路上的运输。这个过程需要专业的方法，以尽量减少停滞、延误和额外费用。对于国际运输和过境运输而言，货运代理尤为重要，因为这些运输需要高度的精确度和协调性。目前，铁路货运代理仍然是最具经济效益的运输方式之一，适用于各种类型的货物，包括空车和载货车厢，以及集装箱。',
      },
      {
        title: '铁路货运代理服务包括哪些内容？',
        text: '我们的运输和货运代理服务涵盖出口、进口和过境运输。我们代理各种类型的货物、空车和载货车厢/集装箱，包括散装货物、液体货物、集装箱货物、危险货物等，运输范围包括乌兹别克斯坦（УТИ）、吉尔吉斯斯坦（КРГ）、土库曼斯坦（ТРК）、塔吉克斯坦（ТДЖ）、哈萨克斯坦（КЗХ）、中国（КНР）、阿富汗（ИГА）、伊朗（ИРА）和俄罗斯（РЖД）。铁路货运代理服务包括铁路运输费用的计算，其中已包括附加费用和安全费用。为此，我们使用专业的计算程序来准确计算货物运输费用。由于每次运输都是独特的，且需要特定的处理方法，我们会考虑运输路线、货物类型、运输车辆，并准备所有必要的文件来跟踪货物运输，并在整个运输过程中协调货物的运输。根据客户需求，我们还可以通过我们的可靠合作伙伴提供货物安全服务',
      },
      {
        title:
          '凭借丰富的经验和高度的专业精神，我们将竭尽全力确保您的运输顺利进行。',
        text: '',
      },
      {
        title: '与我们合作，您将以优惠的条件获得可靠的货运代理合作伙伴！',
        text: '',
      },
    ],
  },

  {
    id: '3',
    language: 'ru',
    created: '11/25/2024',
    title: 'Предоставление ж/д подвижного состава',
    subtitle: 'Предоставление ж/д подвижного состава',
    images: { name: '', file: ServiceImg3 },
    content: [
      {
        title:
          'Что Вам необходимо знать о предоставлении подвижного состава от компании Орион-Транс?',
        text: 'Орион-Транс предоставляет услуги по предоставлению подвижного состава любого типа, подходящего для вашего груза. Вы можете ознакомиться с характеристиками каждого вида вагонов, перейдя к соответствующей иллюстрации.',
      },
      {
        title: '',
        text: 'Мы подберем определенный вагон для перевозки вашего груза, который идеально будет соответствовать вашим требованиям. Особое внимание уделяется проверке состояния вагонов, что позволяет нам подходить к каждому запросу индивидуально. Благодаря своему небольшому парку железнодорожных вагонов и многолетнему сотрудничеству с надежными провайдерами вагонов - мы выстроили крепкие и доверительные отношения. Доверяя нашему опыту, вы экономите время на поиске вагонов и оформлении дополнительных документов.',
      },
      {
        title: 'Почему стоит выбрать нас для аренды подвижного состава?',
        text:
          '1. Плановая подача вагонов - мы обеспечиваем своевременную подачу порожних вагонов под погрузку согласно согласованному графику, что позволяет вам эффективно планировать погрузочно-разгрузочные работы.\n' +
          '2. Оптимальное решение для грузов - арендованный подвижной состав идеально подходит для перевозки сыпучих материалов, промышленных товаров и сырья, а также для любого типа грузов.\n' +
          '3. Экономия ресурсов - аренда вагонов снижает затраты на покупку подвижного состава, а также экономит время на поиски и оформление документов.\n' +
          '4. Доступные тарифы - мы предлагаем конкурентоспособные цены на аренду вагонов, выгодные для вашего бизнеса.\n' +
          '5. Надежные партнерства - сотрудничество с крупными транспортными компаниями позволяет нам предоставлять клиентам стабильные и качественные услуги.\n' +
          '6. Гибкие условия аренды - возможность долгосрочной аренды и другие формы предоставления вагон избавляет вас от необходимости искать вагоны в условиях высокого спроса.',
      },
    ],
  },
  {
    id: '3',
    language: 'en',
    created: '11/25/2024',
    title: 'Provision of Rolling Stock',
    subtitle: 'Provision of Rolling Stock',
    images: { name: '', file: ServiceImg3 },
    content: [
      {
        title: '',
        text: 'Orion-Trans offers the provision of rolling stock of various types such as gondola cars, covered wagons, platforms, specialized wagons, and others. Our specialists will assign the right rolling stock specifically to your cargo.',
      },
      {
        title:
          "What you should know about Orion-Trans' provision of rolling stock services.",
        text: 'OrionTrans offers you services for the provision of rail transportation with rolling stock of any type (gondola cars, platforms, covered wagons, specialized rolling stock) to transport your goods across nations. Our specialists, which include operators of almost all types of railway rolling stock, will select the rolling stock that is optimally suited for the transportation of your cargo and organize transportation by rail.',
      },
      {
        title: 'What are the advantages of rolling stock?',
        text: 'Rolling stock provides a planned supply of empty wagons for loading according to an agreed schedule which allows free time for loading and unloading of goods. The railway today continues to be a key carrier of bulk cargo. With the help of rail transport, 80% of industrial goods and raw materials could be delivered. Rolling stock is optimally suited for transporting in bulks and cargo, and organizing transportation by rail.',
      },
      {
        title: 'Why use OrionTrans provision of rolling stock?',
        text: "OrionTrans offers the provision of rolling stock of various types such as gondola cars, covered wagons, platforms, specialized wagons, and others. Our specialists will assign the right rolling stock specifically to your cargo. We also assist in coordinating plans for the transportation of goods within the territory of the Russian Federation, through land border crossings, and through ports. Close interaction with a wide network of owners of wagons, containers, cars allows us to guarantee the export of the planned volume according to the client's schedule and favorable rates.",
      },
    ],
  },
  {
    id: '3',
    language: 'zh',
    created: '11/25/2024',
    title: '提供铁路运输车辆',
    subtitle: '提供铁路运输车辆',
    images: { name: '', file: ServiceImg3 },
    content: [
      {
        title: '关于公司提供铁路运输车辆，您需要了解什么',
        text: 'Orion-Trans 可为您的货物提供任何类型的机车车辆。您可以点击相应的插图，了解每种货车的特点。',
      },
      {
        title: '',
        text: '我们将为您的货物运输选择完全符合您要求的特定货车。我们会特别注意检查车厢的状况，这使我们能够逐一满足您的要求。凭借我们的小型铁路货车车队以及与可靠货车供应商的多年合作，我们已经建立了牢固的信任关系。通过信任我们的经验，您可以节省寻找车厢和办理额外手续的时间。',
      },
      {
        title: '为什么选择我们租赁运输车辆？',
        text:
          '1. 计划性调度车厢 - 我们根据商定的时间表，确保按时将空车厢调配到装载地点，这使您能够有效地规划装卸作业。' +
          '2. 货物运输的最佳解决方案 - 租赁车辆非常适合运输散装材料、工业产品和原材料，以及任何类型的货物。' +
          '3. 资源节省 - 租赁车厢减少了购买运输设备的费用，并节省了寻找和办理文件的时间。' +
          '4. 价格合理 - 我们提供对您的业务有利的、具有竞争力的车厢租赁价格。' +
          '5. 可靠的合作伙伴关系 - 与大型运输公司合作使我们能够为客户提供稳定和优质的服务。' +
          '6. 灵活的租赁期限 - 可以长期租赁或以其他形式提供车皮，使您不必在需求旺盛时寻找车皮。',
      },
    ],
  },

  {
    id: '4',
    language: 'ru',
    created: '11/25/2024',
    title: 'Железнодорожные контейнерные перевозки',
    subtitle: 'Железнодорожные контейнерные перевозки',
    images: { name: '', file: ServiceImg4 },
    content: [
      {
        title: 'Что необходимо знать о контейнерных перевозках?',
        text: 'Железнодорожные контейнеры - это идеальное решение для перевозки грузов на дальние расстояния. Этот вид перевозки является одним из самых экономичных, прогрессирующих и удобных. Стоимость на контейнерные перевозки значительно ниже за счет снижения затрат на контейнеры и складских расходов. Также, перевозка контейнерами дает возможность хорошо сэкономить на погрузочно-разгрузочных работах. По этим всем причинам, контейнерные перевозки остаются самым выгодным способом для оперативной транспортировки грузов в больших объемов.',
      },
      {
        title:
          'Почему важно воспользоваться железнодорожными контейнерами в Орион-Транс?',
        text: 'Железнодорожные контейнерные грузоперевозки позволяют перевозить практически любой груз, независимо от объема и веса, в пределах границ и между странами. Контейнерные перевозки очень активно развивается в перевозках грузов из Китая в страны СНГ и обратно в Китай. Кроме того, существует контейнерная доставка сборных грузов – это хороший вариант для небольших партий товара, при котором заказчик оплачивает место в контейнере, занимаемое его грузом.',
      },
      {
        title: 'Как осуществляется перевозка грузов ж/д контейнерами?',
        text: 'Погрузка и разгрузка контейнеров осуществляются с помощью специального оборудования, которое адаптировано к стандартным размерам контейнеров. Механизированная погрузка и разгрузка сокращают сроки доставки и устраняют человеческий фактор.',
      },
      {
        title: 'Контейнерные перевозки, как мультимодальные',
        text: 'Контейнерные перевозки являются оптимальным выбором для мультимодальных перевозок. Одним из главных преимуществ использования контейнеров для транспортировки грузов между Китаем и странами СНГ, включая Кыргызстан, является возможность преодолевать различия в ширине железнодорожных путей. Железнодорожные пути в Китае имеют стандартную колею (1435 мм), в то время как в странах СНГ используется широкая колея (1520 мм). На границе с Китаем, в пограничных переходных пунктах, со странами СНГ контейнера перегружаются с одного состава на другой, или на другой вид транспорта, который позволяет минимизировать затраты на логистику и сокращает время простоя. Этот процесс делает контейнерные перевозки более удобными и эффективными.',
      },
      {
        title: 'Дополнительные преимущества контейнерных перевозок:',
        text:
          '1. Универсальность: Контейнеры подходят для перевозки различных типов грузов, включая сыпучие, сборные, опасные и температурно-чувствительные грузы.\n' +
          '2. Сохранность груза: Контейнеры обеспечивают надежную защиту груза от внешних воздействий, таких как погодные условия и механические повреждения.\n' +
          '3. Экономия:Перевозка контейнерами позволяет сократить расходы на погрузочно-разгрузочные работы и уменьшить вероятность потерь или повреждений груза.\n' +
          '4. Гибкость маршрутов: Возможность сочетания железнодорожного транспорта с автомобильными и другими транспортами делает контейнерные перевозки особенно удобными для мультимодальных маршрутов, например, доставка груза из Китая в СНГ через автомобильные пункты пропуска далее железнодорожным транспортом до морских портов Каспийского и Черноморского морей.',
      },
      {
        title: '',
        text: 'Контейнерные перевозки между Китаем и Кыргызстаном, а также другими странами СНГ - это надежный и экономически выгодный способ доставки грузов, который отвечает требованиям современного бизнеса.',
      },
    ],
  },
  {
    id: '4',
    language: 'en',
    created: '11/25/2024',
    title: 'Railway Container Transportation',
    subtitle: 'Railway Container Transportation',
    images: { name: '', file: ServiceImg4 },
    content: [
      {
        title: 'What you need to know about container transport',
        text: 'Rail containers are the ideal solution for transporting goods over long distances. This type of transport is one of the most cost-effective, progressive, and convenient options available. The cost of container transport is significantly lower due to reduced expenses for containers and warehouse costs. Additionally, container transport offers the advantage of significant savings in loading and unloading operations. For all these reasons, container transport remains the most cost-effective method for the fast transportation of large volumes of cargo.',
      },
      {
        title: 'Why choose rail containers with Orion-Trans?',
        text: 'Rail container transport allows you to transport almost any type of cargo, regardless of its size or weight, both domestically and internationally. Container transport is particularly active for shipments from China to the CIS countries and back to China. Additionally, there is consolidated container shipping, which is a great option for smaller shipments, where the customer pays for the space occupied by their goods in the container.',
      },
      {
        title: 'How is rail container transport organized?',
        text: 'Loading and unloading of containers are carried out using specialized equipment, adapted to standard container sizes. Mechanized loading and unloading reduce delivery times and eliminate the risk of human error.',
      },
      {
        title: 'Rail Container Transport as Part of Multimodal Transport',
        text: 'Container transport is an optimal choice for multimodal transportation. One of the key advantages of using containers for transporting goods between China and the CIS countries, including Kyrgyzstan, is the ability to overcome differences in rail track gauges. In China, the standard track gauge (1435 mm) is used, while in the CIS countries, a wider gauge (1520 mm) is employed. At the border with China, at the transition points, containers are transferred from one train to another or to another form of transport, which helps minimize logistics costs and reduces idle time. This process makes container transport more convenient and efficient.',
      },
      {
        title: 'Additional benefits of container transport:',
        text: 'Versatility: Containers are suitable for transporting various types of goods, including bulk, consolidated, hazardous, and temperature-sensitive cargo. Cargo protection: Containers offer reliable protection from external factors such as weather conditions and mechanical damage. Cost savings: Container transport helps reduce costs related to loading and unloading and minimizes the risk of cargo loss or damage.Flexible routes: The ability to combine rail transport with road and other types of transportation makes container shipping particularly convenient for multimodal routes. For example, shipping goods from China to the CIS countries via road checkpoints, and then using rail transport to reach the Caspian and Black Sea ports.',
      },
      {
        title: '',
        text: 'Container transport between China and Kyrgyzstan, as well as other CIS countries, is a reliable and cost-effective way to deliver goods that meets the needs of modern businesses.',
      },
    ],
  },
  {
    id: '4',
    language: 'zh',
    created: '11/25/2024',
    title: '铁路集装箱运输',
    subtitle: '',
    images: { name: '', file: ServiceImg4 },
    content: [
      {
        title: '关于集装箱运输，您需要了解哪些事项？',
        text: '铁路集装箱运输是长途运输货物的理想解决方案。这种运输方式是最经济、最先进的和最方便的之一。由于减少了集装箱和仓储成本，集装箱运输的费用大大降低。此外，集装箱运输还能显著节省装卸作业的费用。正因如此，集装箱运输依然是大宗货物快速运输中最具成本效益的方式。',
      },
      {
        title: '为什么选择Orian-Trans的铁路集装箱运输很重要？',
        text: '铁路集装箱货物运输几乎可以运输任何类型的货物，无论其体积和重量如何，既可以在国内运输，也可以跨国运输。集装箱运输在中亚国家和中国之间的货物运输中发展迅速。此外，还有集装箱拼箱服务——这对于小批量货物来说是一个很好的选择，客户只需支付其货物占用的集装箱空间费用。',
      },
      {
        title: '如何进行铁路集装箱货物运输？',
        text: '集装箱的装卸通过专用设备进行，这些设备适配了集装箱的标准尺寸。机械化的装卸作业缩短了交货时间，并消除了人为因素的影响。',
      },
      {
        title: '集装箱运输作为多式联运',
        text: '集装箱运输是多式联运的最佳选择。使用集装箱运输货物在中国与独联体国家之间，特别是包括吉尔吉斯斯坦在内的国家，最大的优势之一是能够克服铁路轨距的差异。中国的铁路使用标准轨距（1435毫米），而独联体国家则使用宽轨距（1520毫米）。在中国与独联体国家的边境过境点，集装箱会从一列火车换到另一列，或换成其他交通方式，从而最大限度地降低物流成本并减少停运时间。这个过程使集装箱运输更加方便和高效。',
      },
      {
        title: '集装箱运输的额外优势：',
        text:
          '1. 多功能性：集装箱适用于运输各种类型的货物，包括散装货物、拼货、危险品和温度敏感货物。' +
          '2. 货物安全：集装箱提供可靠的保护，防止货物受到天气条件和机械损坏等外部因素的影响。' +
          '3. 经济-集装箱运输使您可以降低装卸操作的成本，并减少货物丢失或损坏的可能性。' +
          '4. 路线灵活性：铁路运输与公路和海运相结合的可能性使得集装箱运输对于多式联运特别方便，例如，通过港口从中国运往独联体。',
      },
      {
        title: '',
        text: '中国与吉尔吉斯斯坦以及其他独联体国家之间的集装箱运输，是一种可靠且经济划算的货物运输方式，符合现代商业的需求。',
      },
    ],
  },

  // {
  //   id: '5',
  //   language: 'ru',
  //   created: '11/25/2024',
  //   title: 'Ежедневная дислокация вагонов',
  //   subtitle: '',
  //   images: { name: '', file: ServiceImg5 },
  //   content: [
  //     {
  //       title: '',
  //       text: '',
  //     },
  //   ],
  // },
  // {
  //   id: '5',
  //   language: 'en',
  //   created: '11/25/2024',
  //   title: 'Daily Wagon Deployment',
  //   subtitle: '',
  //   images: { name: '', file: ServiceImg5 },
  //   content: [
  //     {
  //       title: '',
  //       text: '',
  //     },
  //   ],
  // },
  // {
  //   id: '5',
  //   language: 'zh',
  //   created: '11/25/2024',
  //   title: '每日车厢位置查询',
  //   subtitle: '每日车厢位置查询',
  //   images: { name: '', file: ServiceImg5 },
  //   content: [
  //     {
  //       title: '',
  //       text: '',
  //     },
  //   ],
  // },

  {
    id: '6',
    language: 'ru',
    created: '11/25/2024',
    title: 'Для кого наши услуги',
    subtitle: 'Для кого наши услуги',
    images: { name: '', file: ServiceImg6 },
    content: [
      {
        title: 'Логистические компании',
        text: 'Если вы логистическая/транспортно-экспедиторская компания, мы станем надежными партнерами для организации сложной железнодорожной перевозки. Сотрудничая с нами, вы сможете улучшить и упростить операцию по железнодорожным грузоперевозкам. Также, если вы являетесь собственником вагонов и вам необходимо для перевозки груза железнодорожные коды по регионам УЗБ, ТДЖ, ТРК, ИРА, России, КЗХ и других стран, то вы можете обратиться к нам.',
      },
      {
        title: 'Строительные компании',
        text: 'Строительные проекты требуют доставки таких материалов, как цемент, сталь и тяжелая техника. Наши железнодорожные грузоперевозки предлагают экономичное и надежное решение для транспортировки строительных материалов прямо на строительные площадки, минимизируя задержки и обеспечивая бесперебойную работу.',
      },
      {
        title: 'Промышленные и производственные предприятия',
        text: 'Для промышленных компаний стабильные цепочки поставок — это основа успешной работы. Мы специализируемся на транспортировке как сырьевых материалов, так и готовой продукции, обеспечивая постоянное и эффективное решение для железнодорожных грузоперевозок, которое соответствует вашим производственным срокам.',
      },
      {
        title: 'Индивидуальные предприниматели',
        text: 'Мы поддерживаем предпринимателей, предлагая гибкие решения для железнодорожных перевозок малых и средних партий товаров. Если вы ищете профессиональную поддержку с конкурентоспособным ценовым предложением для того, чтобы перевозить и работать с большими объемами, наши услуги делают крупномасштабную логистику доступной для компаний любого размера.',
      },
      {
        title: 'Импортеры и экспортеры',
        text: 'Международная торговля может быть сложной, но с нашей экспертизой в железнодорожной логистике мы облегчаем этот процесс. Если вы перевозите товары через границы, вам нужен будет партнер, знакомый с международными правилами грузоперевозок.',
      },
      {
        title: 'Перевозка автомобилей',
        text: 'Транспортировка автомобилей требует точности и аккуратности. Наши железнодорожные услуги обеспечат безопасную и эффективную перевозку автомобилей, будь вы производителем, дилером или частным лицом, отправляющим один автомобиль. Если вы ищете экономичную альтернативу автомобильным перевозкам для длительных расстояний, мы предлагаем вам безопасные и специализированные услуги по транспортировке автомобилей.',
      },
    ],
  },
  {
    id: '6',
    language: 'en',
    created: '11/25/2024',
    title: 'For Whom are our Services?',
    subtitle: 'For whom are our services?',
    images: { name: '', file: ServiceImg6 },
    content: [
      {
        title: 'Logistics Companies',
        text: 'If you are a logistics or freight forwarding company, we can become your reliable partner for organizing complex rail transport operations. By partnering with us, you will be able to improve and streamline your railway freight processes. Additionally, if you are a wagon owner and need rail codes for cargo transportation across the regions of Uzbekistan, Tajikistan, Turkmenistan, Iran, Russia, Kazakhstan, and other countries, feel free to contact us.',
      },
      {
        title: 'Construction Companies',
        text: 'Construction projects require the delivery of materials such as cement, steel, and heavy machinery. Our rail freight services offer a cost-effective and reliable solution for transporting construction materials directly to construction sites, minimizing delays and ensuring smooth operations.',
      },
      {
        title: 'Industrial and Manufacturing Enterprises',
        text: 'For industrial companies, stable supply chains are the foundation of successful operations. We specialize in transporting both raw materials and finished products, providing a constant and efficient rail freight solution that aligns with your production timelines.',
      },
      {
        title: 'Entrepreneurs',
        text: 'We support entrepreneurs by offering flexible solutions for transporting small and medium-sized goods. If you’re looking for professional support with competitive pricing to handle larger volumes, our services make large-scale logistics accessible for businesses of all sizes.',
      },
      {
        title: 'Importers and Exporters',
        text: "International trade can be complex, but with our expertise in rail logistics, we simplify the process. If you're transporting goods across borders, you’ll need a partner who is familiar with international freight regulations.",
      },
      {
        title: 'Vehicle Transport',
        text: 'Transporting vehicles requires precision and care. Our rail services ensure safe and efficient vehicle transportation, whether you are a manufacturer, dealer, or an individual sending just one vehicle. If you are looking for a cost-effective alternative to road transport for long distances, we offer safe, specialized vehicle transport services.',
      },
    ],
  },
  {
    id: '6',
    language: 'zh',
    created: '11/25/2024',
    title: '我们的服务对象',
    subtitle: '我们的服务对象',
    images: { name: '', file: ServiceImg6 },
    content: [
      {
        title: '物流公司',
        text: '如果您是一家物流或货运代理公司，我们将成为您组织复杂铁路运输的可靠合作伙伴。通过与我们合作，您可以优化并简化铁路货物运输流程。此外，如果您是车厢所有者，并需要用于运输的铁路区域代码（如乌兹别克斯坦、塔吉克斯坦、土耳其、伊朗、俄罗斯、哈萨克斯坦等国家的代码），您也可以联系我们。',
      },
      {
        title: '建设公司',
        text: '建筑项目需要运输水泥、钢材和重型机械等材料。我们的铁路货运服务为建设材料的运输提供经济高效且可靠的解决方案，直接将物资送至工地，最大程度减少延误，确保工程顺利进行。',
      },
      {
        title: '工业及制造企业',
        text: '对于工业企业来说，稳定的供应链是成功运营的基础。我们专注于运输原材料及成品，为您提供稳定高效的铁路货运解决方案，以满足您的生产时间表需求。',
      },
      {
        title: '个体企业家',
        text: '我们支持企业家，为其提供灵活的铁路运输解决方案。无论是运输小、中还是大批量货物，我们的服务都可以为您提供专业支持和具竞争力的价格，使大规模物流服务也能被各类企业轻松使用。',
      },
      {
        title: '进口商和出口商',
        text: '国际贸易可能非常复杂，但凭借我们在铁路物流方面的专业知识，我们可以让这一过程变得更加简便。无论是跨境运输还是处理国际货运规则，我们都将是您值得信赖的合作伙伴。',
      },
      {
        title: '汽车运输',
        text: '汽车运输需要精准和细致。我们的铁路服务可为您提供安全高效的汽车运输，无论您是制造商、经销商，还是需要运输单辆汽车的个人。对于长距离运输，如果您希望寻找汽车运输的划算替代方案，我们为您提供安全可靠的专业汽车运输服务。',
      },
    ],
  },

  {
    id: '7',
    language: 'ru',
    created: '11/25/2024',
    title: 'Оформление документов',
    subtitle:
      'Оформление сопроводительных документов для организации перевозки грузов',
    images: { name: '', file: ServiceImg7 },
    content: [
      {
        title: 'Что означает обработка документов',
        text: 'Каждая управляемая отправка имеет ряд документов, которые необходимо обработать. Наша команда позаботится об обработке документов и отправке, поэтому вы можете сэкономить время и деньги, пользуясь нашими услугами.',
      },
      {
        title: 'Список документов в логистике',
        text: 'Документы в логистике можно разделить на следующие категории: a) Накладная - Это самый важный документ в процессе железнодорожных грузоперевозок. Этот документ выдает грузоотправитель. Накладная содержит основную информацию о грузе, а именно характеристики груза, стоимость, количество и вес. Она также имеет оригинальные печати. Накладная с оригинальными печатями создается в 4 экземплярах. Один экземпляр хранится у грузоотправителя, второй экземпляр хранится у грузополучателя, третий экземпляр хранится у перевозчика, а последний четвертый экземпляр хранится у нас, который мы отправим в электронном виде нашему клиенту со счетом на оплату. b) Транспортная накладная - Этот документ содержит важную информацию о сроках перевозки и информацию о станциях перегрузки. Первый экземпляр сопровождает груз до станции назначения и предъявляется получателю. Второй экземпляр остается на станции отправления. c) Договор между грузоотправителем и грузополучателем - Этот документ не является обязательным, однако его оформление помогает избежать негативных ситуаций и проблем.',
      },
      {
        title: '',
        text: 'Все вышеперечисленные документы должны содержать согласованную информацию. Данные в счете должны совпадать с информацией в транспортных документах и документах, предоставляемых государственными органами. Таким образом, каждый документ представляет часть единой картины груза. Это исключает вопросы со стороны перевозчиков, владельцев груза и таможенных служб.',
      },
    ],
  },
  {
    id: '7',
    language: 'en',
    created: '11/25/2024',
    title: 'Documents Processing',
    subtitle: 'Shipping Documents Processing for Rail Freight Transportation',
    images: { name: '', file: ServiceImg7 },
    content: [
      {
        title: 'Meaning of Document Processing',
        text: 'Every shipment managed has a number of paperwork that must be processed. Our team will take care of the documents processing and shipment, so you can save time and money enjoying our services.',
      },
      {
        title: 'The List of Documents in Logistics',
        text:
          'Documents in logistics can be divided into the following categories: a) Consignment note\n' +
          'It is the most important document in the rail freight process. This document is given by the consigner. Consignment has the basic information about the cargo, which are cargo characteristics, fee,  amount and weight. It also has the original stamps. Consignment with the original stamps are created in 4 copies. One copy is kept by the consigner, the second copy is kept by the consignee, the third copy is kept by the carrier and the last fourth copy is kept by us, further which we will send electronically to our client with an invoice for payment. b) Waybill\n' +
          'This document contains important information about the freight deadlines and the information about trans-shipment stations. They followed the volume and deadline of the freight. This document is sent by the consigned , where the first copy accompanies until the destination station, where it should be shown to the cargo receiver. The second copy should be left at the dispatching station. c) The agreement between cosigner and consignee\n' +
          'This document is not a compulsory one, however they should be prepared to avoid any negative circumstances and problems.',
      },
      {
        title: '',
        text: 'All of the foregoing documents must contain information that is consistent with one another. The information on the invoice must match the information on the transport documents and documents provided by the government. To put it another way, each document should be part of a larger image that depicts the cargo from all angles. There will be no queries from the carriers, cargo owners, or customs services in this situation.',
      },
    ],
  },
  {
    id: '7',
    language: 'zh',
    created: '11/25/2024',
    title: '文件办理',
    subtitle: '货物运输伴随文件办理',
    images: { name: '', file: ServiceImg7 },
    content: [
      {
        title: '什么是文件处理？',
        text: '每一次货物运输都需要处理一系列文件。我们的团队将负责文件的处理和运输，帮助您节省时间和成本，让您更加放心地使用我们的服务。',
      },
      {
        title: '物流文件清单',
        text:
          '物流中的文件可分为以下几类：a) 运单-运单是铁路货物运输过程中最重要的文件，由发货方开具。运单包含关于货物的基本信息，例如货物的特性、价值、数量和重量，同时附有原始印章。附有原始印章的运单需制作四份副本：\n' +
          ' 1. 一份由发货方保存；\n' +
          ' 2. 一份由收货方保存；\n' +
          ' 3. 一份由承运方保存；\n' +
          ' 4. 最后一份由我们保存，并以电子版形式连同付款账单发送给客户。b) 运输单-该文件包含关于运输时间以及中转站的重要信息。\n' +
          ' 1. 第一份副本随货物一同运至目的地车站，并交由收货人。\n' +
          ' 2. 第二份副本留存于发货车站。c) 发货人与收货人之间的合同-此文件虽非强制性，但其签署有助于避免负面情况和潜在问题的发生。',
      },
      {
        title: '',
        text: '所有上述文件中的信息必须保持一致。账单中的信息需与运输文件以及提交给政府机构的文件信息相符。因此，每份文件都代表着货物整体信息的一部分。这种一致性能够避免承运方、货主以及海关部门提出疑问。',
      },
    ],
  },

  {
    id: '8',
    language: 'ru',
    created: '11/25/2024',
    title: 'Консультации',
    subtitle: 'Индивидуальное консультирование по железнодорожным перевозкам',
    images: { name: '', file: ServiceImg8 },
    content: [
      {
        title: '',
        text: 'Компания Orion-Trans зарекомендовала себя как надежный партнер в области грузоперевозок с 2012 года. За эти годы мы успешно предоставляли экспертные консультации и выстраивали прочные отношения с ведущими транспортными компаниями мира. Наши бизнес-процессы направлены на предоставление полного спектра качественных услуг, чтобы удовлетворить любые потребности в логистике.',
      },
      {
        title: '',
        text: 'Вы можете обратиться к нам для получения бесплатной консультации, адаптированной под ваши потребности.',
      },
      {
        title: '',
        text: 'Пандемия изменила наши подходы к консультациям: мы готовы оказывать поддержку как в онлайн, так и в офлайн-формате. Независимо от того, нужна ли вам помощь с грузоперевозками или логистическими решениями, специалисты Orion-Trans предоставят надежные рекомендации.',
      },
      {
        title: '',
        text: 'Чтобы записаться на бесплатную консультацию, заполните форму или свяжитесь с нами в WhatsApp. Мы подберем удобное для вас время.',
      },
      {
        title: '',
        text: 'Позвоните или напишите нам, и мы обеспечим быструю и безопасную доставку вашего груза до места назначения!',
      },
    ],
  },
  {
    id: '8',
    language: 'en',
    created: '11/25/2024',
    title: 'Consultation',
    subtitle: 'Individual Consultation on Railway Transportation',
    images: { name: '', file: ServiceImg8 },
    content: [
      {
        title: '',
        text: 'Orion-Trans has been a trusted name in cargo transportation since 2012. Over the years, we have successfully provided expert consultations and established strong partnerships with leading transport companies worldwide. Our business processes are designed to deliver a comprehensive range of high-quality services, ensuring that we meet your every logistics need.',
      },
      {
        title: '',
        text: 'By reaching out to us, you can receive a free consultation tailored to your specific requirements.',
      },
      {
        title: '',
        text: "The pandemic has shaped our consultation services, and our team is now fully equipped to offer support both online and offline. Whether you're looking for guidance on freight transport or logistics solutions, Orion-Trans is here to provide you with reliable and expert advice.",
      },
      {
        title: '',
        text: 'To schedule your free consultation, simply fill out the form or contact us directly on WhatsApp. Our specialists will arrange a convenient time for you.',
      },
      {
        title: '',
        text: "Call or message us today, and we'll ensure your cargo reaches its destination quickly and safely!",
      },
    ],
  },
  {
    id: '8',
    language: 'zh',
    created: '11/25/2024',
    title: '咨询',
    subtitle: '铁路运输个性化咨询',
    images: { name: '', file: ServiceImg8 },
    content: [
      {
        title: '',
        text: 'Orion-Trans 公司自2012年以来一直是货运领域值得信赖的合作伙伴。多年来，我们成功提供专业咨询，并与世界领先的运输公司建立了稳固的合作关系。我们的业务流程旨在提供全方位的优质服务，以满足客户在物流领域的任何需求。',
      },
      {
        title: '',
        text: '您可以联系我们，获得根据您的需求的免费咨询服务。',
      },
      {
        title: '',
        text: '疫情改变了我们提供咨询服务的方式：我们随时准备通过线上或线下形式为您提供支持。无论您需要货运帮助还是物流解决方案，Orion-Trans 的专家都将为您提供可靠的建议。',
      },
      {
        title: '',
        text: '如要预约免费咨询，请填写表单或通过 WhatsApp 联系我们。我们将为您安排方便的时间。',
      },
      {
        title: '',
        text: '致电或发送消息给我们，我们将确保您的货物快速、安全地送达目的地！',
      },
    ],
  },
];
